...
class JsonToMd:
    ...
    @rule
    def block_child_database(self, value, prv=None, nxt=None):
        if isinstance(value, dict) and value.get("type", "") == "child_database":
            href = value['id']
            label = value.get("child_database", {}).get("title", "Untitled")
            return f"[{label}]({href})"
        return noop
